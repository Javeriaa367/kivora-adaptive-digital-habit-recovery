...
class Config:
    ...
    # Founder / Super Admin account. Any user whose email matches this value
    # is elevated to role='super_admin' (the highest permission level --
    # inherits every admin capability plus role/permission management) on
    # boot, on signup, and on login. Leave blank ("" ) for a plain
    # admin/user deployment. Lowercased + stripped so a trailing space or a
    # mixed-case address in the .env can't silently fail to match.
