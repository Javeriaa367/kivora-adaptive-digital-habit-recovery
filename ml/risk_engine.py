...
def compute_risk_profile(user_id: int, persist: bool = True) -> dict:
    ...
    #
    # This is tracked PER CATEGORY, not just app-wide: digital_addiction is
    # scored purely from `predictions` (see _score_digital_addiction), so a
    # user who has only ever journaled -- zero assessments taken -- should
    # still see "not enough data" for that one category specifically, even
    # though they have plenty of evidence for depression/anxiety/loneliness.
    has_entries = bool(entries)
    has_predictions = bool(predictions)
    has_checkins = any(checkins_by_week)
    category_evidence = {
        "depression": has_entries or has_predictions,
        "burnout": has_checkins or has_entries or has_predictions,
        "anxiety": has_entries or has_predictions,
        "digital_addiction": has_predictions,
        "loneliness": has_entries or has_predictions,
    }
    ...
    for category, fn in scorers.items():
        ...
        explanation = generate_risk_explanation(category, level, reasons, category_evidence[category])
        profile[category] = {
