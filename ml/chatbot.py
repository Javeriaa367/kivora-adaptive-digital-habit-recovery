...
# ---- Request timeout -------------------------------------------------------
# The client previously had NO timeout configured at all, which means a
# slow, rate-limited, or degraded Gemini endpoint (including the SDK's own
# automatic retry-with-backoff on 429/5xx) could hang for a long time before
# the existing try/except fallback ever kicked in. Journal submission chains
# up to 3 of these calls back-to-back (emotion explanation, memory
# extraction, pattern insight) and the Risk Insights page chains up to 5
# (one per category) -- so an unbounded per-call hang compounds into very
# real, user-visible page lag on exactly those two pages, even though every
# call already degrades safely once it DOES fail.
# Value is in milliseconds (google-genai's http_options convention). Note:
# google-genai rejects per-request deadlines shorter than 10s ("Manually set
# deadline 8s is too short. Minimum allowed deadline is 10s."), so the default
# must stay >= 10000 or every Gemini call fails before it starts.
...
def get_gemini_client():
    ...
    if _client is None:
        ...
        try:
            ...
            # Different google-genai versions have accepted slightly
            # different http_options shapes over time. Never let a timeout
            # config mismatch break client construction entirely -- that
            # would take down every Gemini-backed feature at once, which is
            # a far worse outcome than just not having a timeout.
