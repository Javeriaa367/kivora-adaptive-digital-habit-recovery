...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Admin Interactive Demo Mode (templates/recovery_demo.html) sets this
  // so every hardcoded /api/recovery/... call below is served by the
  // admin-protected demo mirror instead of the player endpoints. Empty on
  // the normal player page, so regular behavior is unchanged.
  ...
  function updateProgress(progress) {
    ...
    if (summaryProgress) summaryProgress.textContent = `${progress.percent}%`;
  }

  function updateSummary(summary) {
    if (!summary) return;
    const mechanismEl = document.getElementById('summary-mechanism');
    const workloadEl = document.getElementById('summary-workload');
    const stageEl = document.getElementById('summary-stage');
    const reasonEl = document.getElementById('summary-reason');
    const interventionEl = document.getElementById('summary-next-intervention');
    const analyticsCompletionEl = document.getElementById('analytics-completion');
    const analyticsUsefulnessEl = document.getElementById('analytics-usefulness');
    const analyticsPlansEl = document.getElementById('analytics-plans');
    const analyticsConfidenceEl = document.getElementById('analytics-confidence');
    if (mechanismEl) mechanismEl.textContent = summary.mechanism || 'Your current pattern';
    if (workloadEl) workloadEl.textContent = summary.workload || 'Low';
    if (stageEl) stageEl.textContent = String(summary.stage || 1);
    if (reasonEl) reasonEl.textContent = summary.reason || 'KIVORA is still learning your pattern.';
    if (interventionEl) interventionEl.textContent = summary.next_intervention || 'Your next recovery step is almost ready.';
    if (summary.analytics) {
      if (analyticsCompletionEl) analyticsCompletionEl.textContent = `${Math.round((summary.analytics.completion_rate || 0) * 100)}%`;
      if (analyticsUsefulnessEl) analyticsUsefulnessEl.textContent = `${Number(summary.analytics.usefulness_score || 0).toFixed(1)}/5`;
      if (analyticsPlansEl) analyticsPlansEl.textContent = String(summary.analytics.plan_count || 1);
      if (analyticsConfidenceEl) analyticsConfidenceEl.textContent = summary.analytics.confidence || 'Learning';
    }
  }
  ...
  card.addEventListener('click', async (e) => {
    ...
    const [{ data: activityRes }] = await Promise.all([
      api(`${API_BASE}/api/recovery/activities/${taskId}`),
    ]);
    if (!activityRes.ok) {
    ...
    await api(`${API_BASE}/api/recovery/activities/${taskId}/start`, { method: 'POST' });
  ...
  function renderJournal(activity) {
    ...
    document.getElementById('journal-submit').addEventListener('click', async () => {
      ...
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/journal`, {
  ...
  function renderReflection(activity) {
    ...
    document.getElementById('reflection-submit').addEventListener('click', async () => {
      ...
      submitBtn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/reflection`, {
  ...
  function renderAssessment(activity) {
    ...
    document.getElementById('assessment-submit').addEventListener('click', async () => {
      ...
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/assessment`, { method: 'POST', body });
      if (!data.ok) {
  ...
  function renderQuiz(activity) {
    ...
    document.getElementById('quiz-submit').addEventListener('click', async () => {
      ...
      submitBtn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/quiz`, {
        method: 'POST', body: { responses },
      });
      if (!data.ok) {
  ...
  function renderBreathing(activity) {
    ...
    document.getElementById('breathing-done').addEventListener('click', async (e) => {
      ...
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/breathing`, {
        method: 'POST',
  ...
  function renderTimer(activity) {
    // 10 minutes on the player page (matches the plan copy's "10-minute"
    // tasks); a 30-second countdown in Interactive Demo Mode so a judge
    // can complete the screen-free challenge in reasonable time. The
    // server clamps to >= 30s either way, so the demo still enforces a
    // real wall-clock wait.
    const plannedSeconds = API_BASE ? 30 : 600;
    ...
    document.getElementById('timer-complete').addEventListener('click', async () => {
      ...
      btn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/timer`, {
  ...
  function renderCheckin(activity) {
    ...
    document.getElementById('checkin-submit').addEventListener('click', async () => {
      ...
      submitBtn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/checkin`, {
        method: 'POST',
        body: {
  ...
  function renderHabit(activity) {
    ...
    document.getElementById('habit-submit').addEventListener('click', async () => {
      ...
      submitBtn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/habit`, {
  ...
  function renderAiConversation(activity) {
    ...
    document.getElementById('ai-finish').addEventListener('click', async () => {
      ...
      btn.disabled = true;
      const { data } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/ai-conversation`, { method: 'POST' });
      if (!data.ok) {
  ...
  async function renderProgressReview(activity) {
    ...
    const { data } = await api(`${API_BASE}/api/recovery/plans/${planId}/progress-review`);
    if (!data.ok) {
    ...
    document.getElementById('review-done').addEventListener('click', async () => {
      const { data: res } = await api(`${API_BASE}/api/recovery/activities/${activity.id}/progress-review`, { method: 'POST' });
  ...
  async function refreshPlanState() {
    // Demo mode: this would hit the REAL /api/recovery/active (which can
    // auto-create the admin's own plan) -- the demo page reloads after
    // every completion instead, so there is nothing to refresh here.
    if (API_BASE) return;
    const { data } = await api('/api/recovery/active');
    if (data.ok) {
      updateProgress(data.plan.progress);
  ...
  document.querySelectorAll('.start-plan-btn').forEach((btn) => {
    btn.addEventListener('click', async () => {
      // Demo page has no plan-switch buttons; guard anyway so a stale
      // click can never switch the real admin's plan from demo mode.
      if (API_BASE) return;
