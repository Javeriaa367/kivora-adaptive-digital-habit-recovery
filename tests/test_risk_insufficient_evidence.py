...
def test_journal_only_user_gets_per_category_evidence_distinction(app, make_user):
    """A user who has ONLY ever journaled (zero assessments, zero check-ins)
    has real evidence for depression/anxiety/loneliness, but digital_addiction
    is scored purely from `predictions` -- which this user has none of. That
    one category should still say "not enough data", not "no elevated risk",
    even though the others correctly have real evidence."""
    from ml.emotion_analyzer import analyze_journal_entry
    from database.db import save_journal_entry

    user_id = make_user()
    with app.app_context():
        analysis = analyze_journal_entry("Just an ordinary quiet day, nothing much happened either way.")
        save_journal_entry(user_id, "Just an ordinary quiet day, nothing much happened either way.", analysis)
        profile = compute_risk_profile(user_id, persist=False)

    assert "not enough" in profile["digital_addiction"]["explanation"].lower()
    # The categories that DO read journal entries have real evidence now,
    # so they must keep the original reassurance wording, not "not enough".
    for category in ("depression", "anxiety", "loneliness"):
        assert "not enough" not in profile[category]["explanation"].lower()
